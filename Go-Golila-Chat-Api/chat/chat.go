...
import (
	...
	request_handler "chat_server/request_handler"
)
...
func (c *Client) readPump() {
	...
	for {
		...
		go request_handler.SendToAPIServer(result["access_token"], c.room.roomId, result["message"])
...
func Start(c echo.Context) error {
	...
	if _, ok := roomPool[roomId]; !ok {
		...
		fmt.Println("length: ", len(roomPool))
		fmt.Println(roomPool)
	} else {
